...
```html
<table class="proctab" ...inline styles...>
  <thead>...</thead>
  <tbody>...</tbody>
</table>
```
...
Fragment mode **still emits the full `proctab-*` class hierarchy**
...
<html><head>
  <meta charset="utf-8">
  <title>{table title or "proctab table"}</title>
...
All class names use the `proctab-` prefix to avoid collision with
...
```html
<table class="proctab">
  <caption class="proctab-caption">{table.meta["title"]}</caption>
  <thead>
    ...
    <tr>
      <th rowspan="N_HEADER_ROWS" class="proctab-corner" aria-hidden="true"></th>
      <th colspan="..." scope="colgroup" class="proctab-col-data">...</th>
      ...
      <th colspan="..." scope="colgroup" class="proctab-col-total">Total</th>
    </tr>
  ...
  <tbody>
    ...
    <tr class="proctab-group-header">
      <th scope="rowgroup" class="proctab-row-label">West</th>
      <td colspan="{n_body_cols}" class="proctab-group-pad"></td>
    </tr>
    <tr class="proctab-data">
      <th scope="row" class="proctab-row-label proctab-indent-{depth-1}">label</th>
      <td class="proctab-cell proctab-data" data-value="42.0">42</td>
      ...
    </tr>
    <tr class="proctab-subtotal">
      <th scope="row" class="proctab-row-label">West Subtotal</th>
      <td class="proctab-cell proctab-subtotal" data-value="...">...</td>
      ...
    </tr>
    <tr class="proctab-total">
      <th scope="row" class="proctab-row-label">Grand Total</th>
      <td class="proctab-cell proctab-total">...</td>
      ...
    </tr>
  </tbody>
  <tfoot>
    <!-- Source + footnotes, present iff table.meta has them -->
    <tr class="proctab-source">
      <td colspan="{n_total_cols}">Source: {table.meta["source"]}</td>
    </tr>
    <tr class="proctab-footnote">
...
- `title` → `<caption class="proctab-caption">…</caption>` immediately
  inside `<table>`. Omitted when `meta` has no title.
- `source` → `<tr class="proctab-source"><td colspan=…>Source: …</td></tr>`
  in `<tfoot>`. Omitted when no source.
- `footnotes` → one `<tr class="proctab-footnote">` per footnote, in
...
- The top-left corner gets
  `<th rowspan="N_HEADER_ROWS" class="proctab-corner" aria-hidden="true">`
...
- Headers carry a class per role: `proctab-col-data`,
  `proctab-col-subtotal`, or `proctab-col-total`.
...
- `class="proctab-indent-{depth-1}"` for CSS targeting.
- Interior row-tree nodes (group headers like "West" above its product
  leaves) emit a `<tr class="proctab-group-header">` with TWO cells:
...
- `class="proctab-cell proctab-{role}"` where role is `data` /
...
Non-PRESENT cells also get `class="proctab-cell proctab-missing-{reason}"`
(e.g., `proctab-missing-empty`, `proctab-missing-null`,
`proctab-missing-not-applicable`, `proctab-missing-suppressed`) for CSS
...
<table class="proctab">
  <thead>
    <tr>
      <th rowspan="1" class="proctab-corner" aria-hidden="true"></th>
      <th scope="col" class="proctab-col-data">N</th>
      <th scope="col" class="proctab-col-data">Pct</th>
      <th scope="col" class="proctab-col-data">CumN</th>
      <th scope="col" class="proctab-col-data">Cum%</th>
    </tr>
  </thead>
  <tbody>
    <tr class="proctab-data">
      <th scope="row" class="proctab-row-label">West</th>
      <td class="proctab-cell proctab-data" data-value="45">45</td>
      <td class="proctab-cell proctab-data" data-value="30">30.0%</td>
      <td class="proctab-cell proctab-data" data-value="45">45</td>
      <td class="proctab-cell proctab-data" data-value="30">30.0%</td>
    </tr>
    <!-- East, South, North rows ... -->
    <tr class="proctab-total">
      <th scope="row" class="proctab-row-label">Total</th>
      <td class="proctab-cell proctab-total" data-value="150">150</td>
      ...
    </tr>
  </tbody>
</table>
...
<thead>
  <tr>
    <th rowspan="2" class="proctab-corner" aria-hidden="true"></th>
    <th colspan="4" scope="colgroup" class="proctab-col-data">Widget A</th>
    <th colspan="4" scope="colgroup" class="proctab-col-data">Widget B</th>
    <th colspan="4" scope="colgroup" class="proctab-col-total">Total</th>
  </tr>
  <tr>
    <th scope="col" class="proctab-col-data">N</th>
    <th scope="col" class="proctab-col-data">Row%</th>
    <th scope="col" class="proctab-col-data">Col%</th>
    <th scope="col" class="proctab-col-data">Tot%</th>
...
<tbody>
  <tr class="proctab-group-header">
    <th scope="rowgroup" class="proctab-row-label proctab-indent-0">West</th>
    <td colspan="{n_body_cols}" class="proctab-group-pad"></td>
  </tr>
  <tr class="proctab-data">
    <th scope="row" class="proctab-row-label proctab-indent-1">Widget A</th>
    <td>...</td>...
  </tr>
  <tr class="proctab-data">
    <th scope="row" class="proctab-row-label proctab-indent-1">Widget B</th>
    <td>...</td>...
  </tr>
  <tr class="proctab-subtotal">
    <th scope="row" class="proctab-row-label proctab-indent-1">West Subtotal</th>
    <td>...</td>...
  </tr>
  <!-- East rows ... -->
  <tr class="proctab-total">
...
3. ~~CSS-class naming convention~~ Resolved: use `proctab-` prefix
   uniformly (`proctab-cell`, `proctab-total`, etc.) in v0.1. Even with
   `class="proctab"` on the table root, bare names like `data` or
...
4. ~~Group-header rows for interior row-tree nodes~~ Resolved: two
   ...
   plus `<td colspan="{n_body_cols}" class="proctab-group-pad"></td>`.
...
1. **H1.** Module skeleton + format-resolution helper. Create
   `src/proctab/render/html.py` with `render_html(table, *,
...
2. **H2.** Column header rendering. One `<tr>` per col-axis dim
   ...
   corner gets `<th rowspan="n_header_rows" class="proctab-corner"
   aria-hidden="true">` (no `scope=`). All classes use the `proctab-`
   prefix per role.
3. **H3.** Body rendering — single ticket combining row-tree
   ...
   cells.
4. **H4.** Caption + tfoot. Emit `<caption class="proctab-caption">`
   from `table.meta.get("title")`; emit `<tfoot>` with one
   `<tr class="proctab-source">` per `source` and one
   `<tr class="proctab-footnote">` per `footnote`. Omit `<caption>`
...
5. **H5.** Default styling. Embedded `<style>` block for standalone
   ...
   scoped selectors like `.proctab-cell`, `.proctab-total`, etc.).
...
8. **H8.** Tests. Parse output with `html.parser` or `xml.etree`
   ...
   `scope` per header, `proctab-` class hierarchy per role,
